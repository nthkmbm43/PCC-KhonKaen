...
const getAdminProduct = unstable_cache(async (id: string) => db
  .select({ product: products, seo: seoMetadata })
  .from(products)
  .leftJoin(
    seoMetadata,
    and(eq(seoMetadata.resourceId, products.id), eq(seoMetadata.resourceType, "product"))
  )
  .where(eq(products.id, id))
  .limit(1), ["admin-product-by-id"], { tags: ["products"], revalidate: 3600 });
