    ...
    <div className={`relative py-24 overflow-hidden flex flex-col ${alignmentClass}`} data-analytics={`cta-block-${headline ? headline.substring(0, 15) : 'default'}`}>
      {backgroundImage && (
        <div className="absolute inset-0 z-0">
          <Image src={backgroundImage} alt="" fill quality={75} sizes="100vw" className="object-cover" />
      ...
      <div className={`relative z-10 max-w-4xl mx-auto px-4 ${alignmentClass} w-full flex flex-col`}>
        ...
        <div className={`flex w-full ${flexAlignmentClass}`}>
          <Link href={actualButtonUrl} className="inline-block bg-brand-600 hover:bg-brand-700 text-white font-bold text-lg px-8 py-4 rounded-xl shadow-[0_8px_20px_rgba(37,99,235,0.4)] hover:shadow-[0_12px_25px_rgba(37,99,235,0.6)] hover:-translate-y-1 transition-all">
            {actualButtonText}
          </Link>
        </div>
      </div>
