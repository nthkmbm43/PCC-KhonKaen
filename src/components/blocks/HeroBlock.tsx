...
export default function HeroBlock({ data }: HeroBlockProps) {
  const { heading, subheading, backgroundImage, buttons, links, headline, description, ctaText, ctaHref, secondaryCtaText, secondaryCtaHref, layout, overlayOpacity, badgeText } = data || {}
  ...
  const opacityClass = overlayOpacity !== undefined ? `opacity-[${overlayOpacity}%]` : 'opacity-30';
  ...
  return (
    <section className="relative overflow-hidden bg-slate-900 pt-20 pb-24 group sm:pt-28 sm:pb-32 lg:pt-40 lg:pb-48">
      ...
      <div className={`relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col ${alignmentClass} animate-fade-up`}>
        ...
        {actualSubheading && (
          <p className="mt-6 text-lg text-slate-300 max-w-3xl mb-10 leading-relaxed drop-shadow-md sm:text-xl lg:mb-14">
            {actualSubheading}
          </p>
        )}
        
        <div className={`flex flex-col sm:flex-row w-full gap-5 sm:gap-6 ${flexAlignmentClass}`}>
          {ctaText ? (
          ...
          {secondaryCtaText ? (
            <Link href={secondaryCtaHref || "#"} className="bg-white/10 hover:bg-white/15 text-white backdrop-blur-md border border-white/20 px-8 py-4 rounded-full font-bold text-lg flex items-center justify-center gap-3 transition-all duration-300 hover:scale-105 shadow-[0_8px_30px_rgba(0,0,0,0.1)] group">
