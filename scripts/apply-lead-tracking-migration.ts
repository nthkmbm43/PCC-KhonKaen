...
const statements = [
  ...
  `UPDATE "leads" SET "lead_code" = 'KK-' || to_char("created_at" AT TIME ZONE 'Asia/Bangkok', 'YYYYMMDD') || '-' || upper(substring(replace("id"::text, '-', '') from 1 for 6)) WHERE "lead_code" IS NULL`,
...
async function main() {
  ...
  const result = await sql.query(`
    ...
    WHERE table_schema = 'public' AND table_name = 'leads'
      AND column_name IN ('lead_code', 'handoff_status', 'confirmed_area_sqm', 'commission_amount', 'updated_at')
  `);

  if (result.rows.length !== 5) throw new Error(`Lead tracking migration verification failed: found ${result.rows.length}/5 key columns`);
  console.log('Lead tracking migration ready (5 key columns verified).');
}
